...
void MMU::AlterStackTop (short int offset)
{
	...
	__assert (new_size >= 0, "Unable to alter stack top by %hd: current top is %zu, new top is negative",
...
Command& MMU::ACommand (size_t ip)
{
	...
	__assert (ip < CurrentBuffer().commands.size(),
...
calc_t& MMU::AData (size_t addr)
{
	...
	__assert (addr < CurrentBuffer().data.size(),
...
{
	verify_method;
	__assert (image, "NULL section image pointer");
	...
	case SEC_SYMBOL_MAP:
		__asshole ("Cannot do binary operations on symbol map section");
		break;
...
void MMU::VerifyReference (const Processor::DirectReference& ref) const
{
	...
	{
	case S_CODE:
		__verify (ref.address < CurrentBuffer().commands.size(),
				  "Invalid reference [TEXT:%zu] : limit %zu", ref.address, CurrentBuffer().commands.size());
		break;

	case S_DATA:
		__verify (ref.address < CurrentBuffer().data.size(),
