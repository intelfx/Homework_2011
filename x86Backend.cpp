...
// Encode register to Reg or R/M field
unsigned char RegEncode(x86Regs reg)
{
	switch( reg ) {
	case RAX:
		return 0;
	...
	case RDI:
		return 7;
	default:
		s_casshole( "Invalid register to R/M" );
	}
}
...
unsigned char ModRM( unsigned char Mod, unsigned char RegOrDigit, unsigned char RM )
{
	return ( Mod << 6 ) | ( RegOrDigit << 3 ) | RM;
}
...
unsigned char RexPrefix( mask_t flags )
{
	return 64 | flags; // rex is 0100WR0B, so 64 | flags
}
